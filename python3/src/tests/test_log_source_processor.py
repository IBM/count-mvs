...
def test_excluded_log_sources():
    ...
    assert (list(mvs_results.get_device_map().keys()))[0] == '2.2.2.2'
...
def test_multiple_domains():
    ...
    assert '1.1.1.1' in list(mvs_results.get_device_map().keys())
    assert '2.2.2.2' in list(mvs_results.get_device_map().keys())
    assert 'Test Domain One' in list(mvs_results.get_domain_count_map().keys())
    assert 'Test Domain Two' in list(mvs_results.get_domain_count_map().keys())
    assert 'Test Domain Three' in list(mvs_results.get_domain_count_map().keys())
    assert 'Test Domain Four' in list(mvs_results.get_domain_count_map().keys())
...
def test_windows_workstations_removed():
    ...
    with patch('countMVS.WindowsDeviceProcessor.get_windows_workstations', return_value=windows_workstations):
        ...
        assert '127.0.0.1' not in list(mvs_results.device_map.keys())
...
def test_hostname_to_ip_resolution_with_no_ip():
    with patch('countMVS.IPParser.get_device_ip', return_value=None):
        ...
        mvs_results = processor.get_mvs_results()
        assert mvs_results.get_mvs_count() == 2
        assert len(list(mvs_results.get_device_map().keys())) == 1
        assert list(mvs_results.get_device_map().keys())[0] == 'microsoft.test.com'
